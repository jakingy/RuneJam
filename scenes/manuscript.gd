...
var _auto_follow_scroll: bool = false
var _programmatic_scroll: bool = false
const SCROLL_FOLLOW_MARGIN := 24.0
...
func run_debug_test() -> void:
	...
	add_analysis_message("This is an analysis message used for debugging.")
...
func begin_auto_follow_scroll() -> void:
	_auto_follow_scroll = true
	scroll_to_bottom()


func stop_auto_follow_scroll() -> void:
	_auto_follow_scroll = false


func _on_scroll_value_changed(_value: float) -> void:
	if _programmatic_scroll:
		return

	if _auto_follow_scroll and not is_near_scroll_bottom():
		_auto_follow_scroll = false


func is_near_scroll_bottom() -> bool:
	if not is_instance_valid(manuscript_scroll):
		return true
	...
	return (v_scroll.max_value - float(v_scroll.value)) <= SCROLL_FOLLOW_MARGIN
